...
def create_app(test_config=None):
    ...
    def get_actors(jwt):
        try:
            return jsonify({'success': True, 'actors': paginate_response(
                request, Actor.query.order_by(Actor.id).all())})
    ...
    def create_actor(jwt):
        ...
        # Abort 400 if the name in the request matches any of the saved actor
        # names
        if name in list(map(Actor.get_name, Actor.query.all())):
            abort(
                400,
                'This name is already taken. '
                'Please provide a new name and try again.')
    ...
    def create_movie(jwt):
        ...
        # Abort 400 if the title in the request matches any of the saved movie
        # titles
        if title in list(map(Movie.get_title, Movie.query.all())):
            abort(
                400,
                'This title is already taken. '
                'Please provide a new title and try again.')
